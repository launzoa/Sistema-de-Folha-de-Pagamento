...
Esta seção descreve os requisitos funcionais do SFP-CLT, organizados por módulo de processamento. Cada requisito é expresso em linguagem verificável, acompanhado das regras de negócio derivadas da legislação CLT, das fórmulas matemáticas aplicáveis e, quando pertinente, de exemplos numéricos para eliminar ambiguidades de interpretação.
...
**3.2.1.2.2 —** Campos opcionais: Número de Dependentes (inteiro ≥ 0, padrão 0), banco e conta-corrente para depósito, e-mail institucional.
...
**3.2.1.2.6 —** O sistema deve identificar o mês de admissão de cada funcionário para aplicar cálculo proporcional automaticamente na primeira folha processada após a data de admissão.

**3.2.1.2.7 —** A inativação de um funcionário deve registrar obrigatoriamente a data de desligamento. Funcionários inativos não participam do processamento de folhas de meses posteriores à data de desligamento.
...
**3.2.1.3.2 —** O sistema deve incluir as rubricas padrão pré-cadastradas conforme Apêndice 4.1, sem permitir exclusão das rubricas com códigos 001 a 005 (essenciais ao processamento).
...
**3.2.1.4.2 —** Deve existir ao menos um usuário com perfil Administrador ativo em todo momento. ***O sistema deve impedir a inativação do último Administrador <- Analisar a possibilidade.
...
**3.2.2.1 —** O sistema deve calcular, para cada mês de competência, o total de dias úteis com base no calendário gregoriano, considerando dias úteis como segunda a sexta-feira, excluindo sábados e domingos.
...
**3.2.2.2.2 —** O desconto por fração de dia (horas) deve ser calculado como: Desconto_Fração = Valor_Hora × Nº_Horas_Faltadas, onde Valor_Hora = Salário_Base ÷ Horas_Mensais_Contratuais.
...
**3.2.2.3 —** O sistema deve permitir o lançamento de atrasos em minutos, associados a uma data específica dentro do mês de competência corrente. **VER SE VAI CALCULAR ATROSO DE MINUTOS TBM OU VAI ARREDONDAR
...
**3.2.2.5.3 —** Os primeiros 15 dias de afastamento por doença são de responsabilidade do empregador (CLT Art. 476). 

**3.2.2.5.3 —** O sistema deve sinalizar ao operador quando um atestado superar 15 dias corridos consecutivos, indicando que o caso deve ser encaminhado ao INSS (auxílio-doença), sem bloquear o lançamento.

**3.2.2.5.4 —** O upload de arquivo digitalizado do atestado (PDF, JPG ou PNG, máx. 5 MB) é opcional nesta versão, ficando o controle documental a cargo do operador. *****VER SE VAI TER ISSO TAMBÉM
...
**3.2.4.4 —** O sistema deve suportar o lançamento de descontos variáveis adicionais no módulo de exceções, como adiantamentos ou outras deduções autorizadas.
	...
	**3.2.4.4.2 —** O sistema deve impedir que o total de descontos supere o total de proventos, resultando em salário líquido negativo. Caso isso ocorra, o processamento deve ser interrompido com mensagem de erro clara ao operador, listando os descontos que causaram o problema.
...
**3.2.6.1 —** O sistema deve gerar o holerite individual de cada funcionário para cada mês de competência processado, exigidas pelo Art. 464 da CLT.
	**3.2.6.1.1 —** O holerite deve ser composto por três blocos obrigatórios:
		**(a)** Cabeçalho: Razão Social e CNPJ da empresa, Nome do funcionário, CPF mascarado, Cargo, Data de Admissão, e Mês/Ano de competência.
		**(b)** Corpo: Tabela de Proventos (código, descrição, referência/quantidade, valor) e Tabela de Descontos (código, descrição, referência/quantidade, valor). Cada rubrica ocupa uma linha.
		**(c)** Rodapé: Total Bruto de Proventos, Total de Descontos, Salário Líquido (em destaque tipográfico), e linha de assinatura do colaborador (versão impressa) ou texto "Documento Digital" (versão PDF).
...
**3.2.6.2 —** O sistema deve gerar, após o fechamento da folha, um relatório de resumo mensal consolidado contendo, para cada funcionário: Nome, Cargo, Salário Bruto, Total de Descontos, Salário Líquido e Status.
...
**3.2.6.3 —** O sistema deve gerar um arquivo de remessa simplificado com os dados necessários para crédito em conta dos funcionários, contendo: CPF, Nome, Banco, Agência, Conta e Valor Líquido.
	**3.2.6.3.1 —** O arquivo deve ser gerado em formato CSV com separador ponto-e-vírgula e codificação UTF-8.
...
**3.2.6.5 —** O sistema deve manter um log de auditoria imutável que registra todas as operações de escrita realizadas na aplicação.
	...
	**3.2.6.5.3 —** O sistema deve disponibilizar tela de consulta ao log de auditoria, acessível somente ao perfil Administrador, com filtros por período, usuário e tipo de operação.
...
|RN-07|Salário Prop. = (Sal. Base ÷ Dias Úteis) × Dias Trabalhados|CLT Art. 487|3.2.3|
|RN-08|Sal. Bruto = Sal. Base Prop. + HE + Bônus (incid.) + Cesta + PLR|CLT Art. 457|3.2.3|
|RN-09|INSS progressivo por faixa (tabela 2025); teto R$ 8.157,41|Port. MPS 2025|3.2.4|
|RN-10|Salário Líquido ≥ R$ 0,00; desconto não pode superar proventos|CLT Art. 462|3.2.4|
|RN-11|Sal. Líquido = Σ Proventos − Σ Descontos|CLT Art. 464|3.2.4|
...
|Processamento da folha — até 50 func.|15 s|Hardware de referência, carga normal|
|Processamento da folha — 51 a 200 func.|45 s|Hardware de referência, carga normal|
...
|Inicialização da aplicação|8 s|Após SO já iniciado, hardware de ref.|
...
**3.3.2.1 —** O sistema deve exigir autenticação por login e senha em toda sessão. 
	...
	**3.3.2.1.3 —** Após 3 tentativas consecutivas de login com senha incorreta para o mesmo usuário, a conta deve ser bloqueada temporariamente por 5 minutos.
...
**3.3.2.2 —** As senhas de todos os usuários devem ser armazenadas no banco de dados exclusivamente como hash bcrypt, com fator de custo (work factor) mínimo de 10, equivalente a aproximadamente 100 ms de processamento por tentativa de autenticação na configuração de hardware de referência.
...
**3.3.2.5 —** O sistema deve manter um log de segurança separado do log de auditoria funcional, registrando exclusivamente eventos de acesso e autenticação.
	**3.3.2.5.1 —** O log de segurança deve registrar: login bem-sucedido, login falho (com usuário informado), bloqueio de conta, encerramento de sessão (manual ou por timeout) e redefinição de senha.
...
**3.3.3.3 —** O sistema deve utilizar transações de banco de dados com propriedades ACID para todas as operações de escrita relacionadas ao processamento da folha. Em caso de erro, o sistema deve executar rollback completo dos dados desse funcionário sem afetar os demais já processados com sucesso.
...
**3.3.3.4 —** O sistema deve validar a integridade referencial dos dados antes de iniciar o processamento da folha. Deve listar todos os problemas encontrados em tela de pré-validação e impedir o processamento até que sejam corrigidos.

**3.3.3.5 —** O sistema deve gerar automaticamente um backup do banco de dados ao término de cada fechamento de folha bem-sucedido, antes de alterar o status da folha para "Fechada".
	**3.3.3.5.2 —** O sistema deve alertar caso o diretório configurado não exista ou não tenha permissão de escrita.
	**3.3.3.5.1 —** O arquivo de backup deve seguir a nomenclatura: SFPCLT_BACKUP_[AAAAMM]_[YYYYMMDD_HHMMSS].sql
...
**3.3.3.6 —** O sistema deve detectar e tratar as seguintes condições de erro de infraestrutura, exibindo mensagem clara ao usuário:
	**(a)** Perda de conexão com o banco de dados durante operação: exibir mensagem, aguardar 30 segundos e tentar reconexão automática antes de encerrar a operação com erro.
	**(b)** Falta de espaço em disco durante exportação de PDF: exibir mensagem específica indicando o problema e o caminho de destino.
	**(c)** Falha de impressora durante impressão de holerite: exibir mensagem de erro com opção de tentar novamente ou cancelar, sem travar a aplicação.
...
Os requisitos desta categoria garantem que a interface seja eficiente, compreensível e tolerante a erros para o perfil de usuário definido. Os critérios seguem as 10 heurísticas de usabilidade de Jakob Nielsen.
...
**3.3.4.4 —** Operações destrutivas ou irreversíveis devem sempre ser precedidas de diálogo de confirmação modal com dois botões claramente distintos: confirmação e cancelamento.
	**3.3.4.4.1 —** Exemplos de operações que exigem confirmação modal: fechamento de folha, inativação de funcionário, alteração de salário base, exclusão de lançamento de exceção, redefinição de senha de usuário.
...
**3.3.6.8 —** O sistema deve incluir documentação técnica mínima: diagrama de arquitetura em camadas, dicionário de dados do banco de dados e instruções de instalação, configuração e atualização. Essa documentação deve ser mantida atualizada a cada release.
...
**3.3.8.1 —** O sistema deve implementar os cálculos de folha de pagamento em conformidade estrita com a legislação CLT vigente para o ano de apuração da competência, utilizando os valores de tabelas (INSS, piso salarial, horas extras) válidos para aquele ano — não os do ano corrente, quando processamentos retroativos forem realizados.
...
**3.3.8.4 —** O sistema deve manter registros de folhas fechadas pelo prazo mínimo de 5 (cinco) anos, conforme determinado pelo Art. 11 da CLT (prescrição trabalhista) e pelo Art. 195 do Código Tributário Nacional.
...
|3.3.1.7|Desempenho|RAM ≤ 512 MB normal \| ≤ 1 GB pico processamento|Média|
|3.3.2.1|Segurança|Nenhum acesso sem autenticação; perfis isolados|Alta|
|3.3.2.1.3|Segurança|Bloqueio de conta após 3 tentativas / 5 min|Alta|
|3.3.2.1.2|Segurança|Sessão encerrada após 30 min de inatividade|Alta|
|3.3.2.2|Segurança|Senhas em bcrypt custo ≥ 10; sem texto plano|Alta|
|3.3.2.3.1|Segurança|CPF mascarado em listagens; LGPD compliance|Alta|
|3.3.3.1|Confiabilidade|BigDecimal 4 casas; arredondamento HALF_UP|Alta|
|3.3.3.2|Confiabilidade|Processamento idempotente (mesmo resultado 2x)|Alta|
|3.3.3.3|Confiabilidade|Transações ACID com rollback por funcionário|Alta|
|3.3.3.5|Confiabilidade|Backup automático em cada fechamento de folha|Alta|
|3.3.4.1|Usabilidade|Usuário treinado: ciclo completo em ≤ 20 min|Média|
|3.3.4.3|Usabilidade|Mensagens de erro em linguagem clara, sem stack trace|Média|
|3.3.5.1|Disponibilidade|Disponibilidade ≥ 99% na janela de expediente|Alta|
|3.3.6.1|Manutenibilidade|Arquitetura 3 camadas separadas (View/Service/DAO)|Alta|
|3.3.6.2|Manutenibilidade|Tabelas legais configuráveis no BD sem recompilação|Alta|
|3.3.6.3|Manutenibilidade|Cobertura de testes unitários ≥ 70% na camada de negócio|Média|
|3.3.6.6|Manutenibilidade|Logs de aplicação rotativos, 30 dias de retenção|Média|
|3.3.7.1|Portabilidade|Execução em Windows 10/11, Ubuntu 20/22, macOS 12+|Alta|
|3.3.8.2|Conformidade|Folha fechada imutável em camada de app e BD|Alta|
|3.3.8.4|Conformidade|Retenção mínima de 5 anos de folhas fechadas|Alta|
...
|002|Hora Extra 50%|Variável|Sim|Sim|Adicional mínimo 50% sobre valor-hora — CLT Art. 59|
|003|Hora Extra 100%|Variável|Sim|Sim|Domingos, feriados ou acordo coletivo — CLT Art. 59|
...
|202|Provisão 13º Salário|Mensal|Salário_Base ÷ 12|Acumulado jan–dez; pago em nov/dez — Lei 4.090/62|
|203|Provisão Férias + ⅓|Mensal|(Sal.Base÷12)×(4/3)|Período aquisitivo 12 meses — CLT Art. 129|
...
|                                                                                                                                                                                                                        |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 📐 **F-01 — Valor do Dia de Trabalho [RN-02]**<br><br>Valor_Dia = Salário_Base ÷ Dias_Úteis_do_Mês<br><br>Exemplo: Salário Base = R$ 3.300,00 \| Dias úteis de março = 21<br><br>Valor_Dia = 3.300,00 ÷ 21 = R$ 157,14 |
...
|Teto|Acima de R$ 8.157,41|—|Contribui sobre R$ 8.157,41|
...
|                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 📐 **F-12 — Provisão Mensal de Férias + ⅓ Constitucional [RN-14]**<br><br>Provisão_Férias_Mês = (Salário_Base ÷ 12) × (4 ÷ 3)<br><br>Detalhado:<br><br>Férias_Mês = Salário_Base ÷ 12<br><br>Adicional_⅓ = Férias_Mês ÷ 3<br><br>Provisão_Total = Férias_Mês + Adicional_⅓<br><br>Exemplo: Salário Base = R$ 3.600,00<br><br>Férias_Mês = 3.600 ÷ 12 = R$ 300,00<br><br>Adicional = 300 ÷ 3 = R$ 100,00<br><br>Provisão = R$ 400,00 / mês |
...
|Férias + ⅓|30 dias de férias anuais acrescidas de ⅓ de adicional constitucional.|CLT Art. 129 / CF Art. 7º|
...
|ACID|Atomicidade, Consistência, Isolamento e Durabilidade. Propriedades de transações de BD.|—|
...
|BigDecimal|Tipo Java de precisão arbitrária usado para cálculos monetários sem erros de ponto flutuante.|—|
|bcrypt|Algoritmo de hash de senha com fator de custo configurável (mínimo 10).|OWASP / RNF 3.3.2|
...
|3.2.2 — Apuração de Frequência|Apuração de Frequência|3.1.6|RN-02, 03, 04, 05, 06|
|3.2.3 — Proventos|Processamento de Proventos|3.1.7|RN-07, RN-08|
|3.2.4 — Descontos|Processamento de Descontos|3.1.7|RN-09, 10, 11|
|3.2.5 — Encargos / Provisões|Encargos e Provisões|3.1.7 (relatório)|RN-12, 13, 14|
...
|RN-01|Piso salarial|Sal. Base ≥ R$ 1.518,00 (2025)|Decreto 12.302/2024|
|RN-02|Valor-Dia|Sal. Base ÷ Dias_Úteis|CLT Art. 64|
|RN-03|Desconto Falta|Valor_Dia × Qtd_Dias|CLT Art. 131|
|RN-04|Desconto Atraso|Valor_Hora × (min÷60); tol. 10 min|CLT Art. 58 §1º|
|RN-05|DSR perdido|Valor_Dia × Nº semanas com falta|Lei 605/1949|
|RN-06|Hora extra 50% / 100%|VH×1,5×h / VH×2,0×h|CLT Art. 59|
|RN-07|Salário proporcional|(Sal.Base÷Dias_Úteis)×Dias_Trab.|CLT Art. 487|
|RN-08|Salário Bruto|Σ Proventos com incidência|CLT Art. 457|
|RN-09|INSS progressivo|Tabela 4.2.3 (teto R$ 8.157,41)|Portaria MPS 2025|
|RN-10|Sal. Líquido ≥ R$ 0,00|Σ Proventos ≥ Σ Descontos|CLT Art. 462|
|RN-11|Salário Líquido|Σ Proventos − Σ Descontos|CLT Art. 464|
|RN-12|FGTS Mensal|Base_FGTS × 8%|Lei 8.036/90 Art. 15|
|RN-13|Provisão 13º / mês|Sal. Base ÷ 12|Lei 4.090/1962|
|RN-14|Provisão Férias+⅓ / mês|(Sal.Base÷12)×(4/3)|CLT Art. 129 / CF Art. 7º|
|RN-15|Folha Fechada imutável|Constraint BD + regra de negócio|CLT Art. 464 / LGPD|
# 5 Índice
1. Introdução
