...
* Integração bancária automática (CNAB240/CNAB400) não faz parte do escopo desta versão do sistema.
...
- **3.1.2.7 —** O sistema deve registrar no log centralizado todas as tentativas de acesso (sucesso ou falha), classificadas como categoria `SEGURANÇA`.
...
- **3.1.4.4** — O sistema deve apresentar formulário de cadastro/edição de funcionário com os seguintes campos obrigatórios:
    ...
    - **3.1.4.4.4** — Data de Admissão (DD/MM/AAAA). Deve ser menor ou igual a data atual do sistema.
...
- **3.1.6.4** — O formulário de criação/edição de rubrica deve conter: Código (numérico, 3 dígitos, o máximo é 999), Descrição (texto, máx. 100 caracteres), Natureza, Tipo e checkboxes de Incidência.
  - **3.1.6.4.1** - A criação de nova rubrica seguirá também: campo numérico inteiro entre 001 e 999; personalizadas ≥ 500; padrão 001–499.
...
- **3.1.8.4** — O sistema deve permitir reprocessar a folha enquanto o status for "Aberta", sobrescrevendo os valores calculados anteriormente após confirmação modal.
...
- **3.1.12.3** — A tela deve disponibilizar filtros por: período, usuário, tipo de evento e categoria (AUDITORIA / SEGURANÇA).
...
- **3.2.1.4.5 —** O nome de usuário deve ser único no sistema. Tentativa de duplicata gera erro.
...
* **3.2.2.4.5 —** Para fins de cálculo do Descanso Semanal Remunerado (DSR), considera-se semana o período compreendido entre **domingo e sábado**, conforme Lei nº 605/1949.
* **3.2.2.4.6 —** A ocorrência de falta injustificada em qualquer dia dentro da semana (domingo a sábado) implica a perda de exatamente 1 (um) DSR correspondente àquela semana.
* **3.2.2.4.7 —** Semanas que cruzam dois meses devem ser tratadas da seguinte forma:
    - O DSR perdido deve ser contabilizado no mês de competência ao qual pertence o sábado da semana correspondente.
* **3.2.2.4.8 —** Para fins de cálculo do DSR, qualquer falta injustificada ocorrida entre segunda e sexta-feira implica a perda do DSR correspondente à semana (domingo a sábado) em que a falta ocorreu.
...
- Dias_Trabalhados = Dias_Úteis − Faltas_Injustificadas − Dias_Afastamento_sem_Cobertura.
...
* **3.2.3.3.2 —** O sistema deve sinalizar ao operador quando um mesmo bônus for lançado por 3 meses consecutivos, conforme CLT Art. 457 § 1º.
    - Alerta não-bloqueante, exibido uma única vez por mês para o mesmo funcionário.
...
* **3.2.6.5.1 —** O sistema deve manter um único log centralizado de eventos (LOG), não sendo permitida a existência de logs separados para auditoria e segurança.
* **3.2.6.5.2 —** O sistema deve manter um log de auditoria imutável que registra todas as operações de escrita realizadas na aplicação.
* **3.2.6.5.3 —** Cada entrada do log deve conter: ID sequencial, data e hora (gravados em UTC, exibidos no fuso configurado do servidor de BD), usuário,Categoria (`AUDITORIA` ou `SEGURANÇA`) , tipo de operação (ex.: `CADASTRO_FUNCIONARIO`, `FECHAMENTO_FOLHA`, `ALTERACAO_SALARIO`), entidade afetada (tabela + ID do registro) e resumo da mudança (valor anterior e novo valor, quando aplicável).
* **3.2.6.5.4 —** O log deve ser append-only: nenhum registro pode ser alterado ou excluído por nenhum usuário, incluindo o Administrador.
* **3.2.6.5.5 —** O sistema deve disponibilizar tela de consulta ao log de auditoria (seção 3.1.12), acessível somente ao Administrador, com filtros por período, usuário e tipo de operação.
...
* **3.3.2.5.1 —** Eventos de segurança devem ser registrados no log centralizado do sistema, utilizando a categoria "SEGURANÇA".
* **3.3.2.5.2 —** São considerados eventos de segurança: login com sucesso, login com falha, bloqueio de conta, encerramento de sessão e redefinição de senha.
* **3.3.2.5.3 —** Não deve existir mecanismo de log de segurança separado do log centralizado.
* **3.3.2.5.4 —** Cada entrada deve conter: timestamp (data e hora gravados em UTC, exibidos no fuso configurado do servidor de BD), tipo de evento, nome de usuário informado e identificador da estação de trabalho (endereço IP local).
* **3.3.2.5.5 —** O log de segurança deve ser somente-inserção (append-only), sem interface de exclusão disponível para nenhum perfil.
...
* **3.3.3.10 —** O sistema deve implementar rotina de backup automático do banco de dados local, com frequência mínima diária, armazenando os arquivos em diretório pelo usuário.
* **3.3.3.11 —** O sistema deve permitir a exportação manual do banco de dados sob demanda, garantindo a integridade dos dados exportados.
* **3.3.3.12 —** O sistema deve definir e documentar os seguintes parâmetros de recuperação:
    - **RTO (Recovery Time Objective):** tempo máximo de recuperação de até 2 horas após falha.
    - **RPO (Recovery Point Objective):** perda máxima de dados limitada a 24 horas.
* **3.3.3.13 —** O sistema deve fornecer mecanismo de restauração do banco de dados a partir de arquivos de backup, com validação de integridade antes da recuperação.
...
* **3.3.5.4 —** Em caso de falha de hardware ou perda de dados, o sistema deve possibilitar recuperação a partir de backup conforme políticas de RTO e RPO definidas em confiabilidade.
...
|PLR|Participação nos Lucros e Resultados. Não integra base de encargos.|Lei 10.101/2000|
...
|3.2.5 — Encargos / Provisões|Processamento|3.1.8 (relatório)|UC-11|RN-12, RN-13, RN-14, RN-15|
...
|RN-06|Hora extra 50% / 100% (VH conforme F-02)|VH × 1,5 × h / VH × 2,0 × h|CLT Art. 59|
